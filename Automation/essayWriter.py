...
def randomText(x, option):
    ...
    while count < numberOfWords:
        if option == 1:
            if turn % 2 == 0:
                text = text + randomWordsBig()
            else:
                text = text + randomCursedWords()
        elif option == 2:
            text = text + randomCursedWords() + ","
...
def randomCursedWords():
    cursedWords = ["fuck", "fuck you", "shit", "piss off", "shithead", "dickhead", "asshole", "son of a bitch",
                   "bastard", "bitch", "damn", "cunt", "bollocks", "bugger", "bloody hell", "choad", "crikey",
                   "get stuffed"]
    r = randint(0, 17)

    return cursedWords[r]
